...
rule filt_ingroup_biallelic_snp_nomissing:
	input:
		vcf = "{config['vcf_dir']}{chrom}{config['vcf_suf']}",
...
rule reannotate_hg19_ancestral_alleles:
    input:
        vcf = "{species}/vf_proc_vcf/{samp}_{chrom}.vcf.gz",
        ancbed = "{config['ancbed']}_{chrom}.bed"
    output:
        tmp_vcf = "{species}/vf_proc_vcf/{samp}_{chrom}_tmp.vcf.gz",
    ...
    shell:
        """
		bcftools annotate -x INFO/AA {input.vcf} -Oz -o {output.tmp_vcf} && \
		bcftools annotate -a {input.ancbed} -h {params.hd_dir}/annots.hdr -c CHROM,FROM,TO,AA {output.tmp_vcf} -Oz -o {output.anc_vcf}
        """

# remove current annotation for AA
# reannotate AA with macaque-like ancestral alleles for the 1000G SNPs,
